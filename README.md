...
`python3` **3.3+** for `shunt edit` / `commit` (and the `install` check), `sha256sum` for `commit`,
...
`--name LABEL` gives the unit a readable name instead of a random one. The label is
lower-cased and every character outside `a-z0-9-` is **replaced** by a dash - not
removed - so `--name "deploy 2.0"` becomes the unit `shunt-deploy-2-0`; leading and
trailing dashes are then trimmed. A label that leaves nothing behind (`--name "..."`,
or one written in a non-Latin script) falls back to the random name, so the job still
starts and still prints its `JOB=` line.
...
Register a host and print the hook line: checks `python3` on the server - and WHICH
`python3`, saying so when it is below what the file helpers need - writes the host
into `~/.config/shunt/shunt.toml`, and tests the connection. A version below the floor,
or no `python3` at all, is **reported, not refused**: the host is still good for
everything except `edit` and `commit`. Nothing is installed on the
